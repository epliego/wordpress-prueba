...
function thb_register_my_menus() {
	register_nav_menus(
		array(
			'nav-menu' => esc_html__( 'Navigation Menu','goodlife' ),
			'footer-menu' => esc_html__( 'Footer Menu','goodlife' ),
			'subheader-menu' => esc_html__( 'Sub Header Menu','goodlife' ),
			'mobile-menu' => esc_html__( 'Mobile Menu','goodlife' )
		)
	);
}
/* Category ID on Menu */
function thb_category_id_on_menu( $classes, $item ) {
...
add_filter( 'nav_menu_css_class', 'thb_category_id_on_menu', 10, 2 );
	...
	*/
	function end_lvl(&$output, $depth = 0, $args = array()) {
	  ...
	  if($depth === 0 && $this->active_megamenu) {
	  	$output.= '<div class="category-children cf">'.$this->mega_menu_content.'</div>';
	...
	public function end_el( &$output, $item, $depth = 0, $args = array() ) {
		if( $depth == 1 && $this->in_sub_menu ) {
			...
			if (in_array($item->object, array('post_tag','category', 'product_cat', 'product_tag', 'post_format'))) {
...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu  {
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
		...
		if ( ! empty( $item->_invalid ) ) {
		    ...
		    $title = sprintf( esc_html__( '%s (Invalid)', 'goodlife'), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
		    $classes[] = 'pending';
		    /* translators: %s: title of menu item in draft status */
		    $title = sprintf( esc_html__('%s (Pending)', 'goodlife'), $item->title );
		}
					...
					<dt class="menu-item-handle">
					    ...
					    <span class="item-controls">
					        ...
					        <span class="item-order hide-if-js">
					            ...
					            ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'goodlife'); ?>">&#8593;</abbr></a>
					            |
					            <a href="<?php
					                echo wp_nonce_url(
					                    add_query_arg(
					            ...
					            ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','goodlife'); ?>">&#8595;</abbr></a>
					        </span>
					        <a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('Edit Menu Item', 'goodlife'); ?>" href="<?php
					            echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
					        ?>"><?php _e( 'Edit Menu Item', 'goodlife'); ?></a>
					    </span>
					</dt>
				</dl>
		  ...
		  <div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
		        ...
		        <p class="description description-thin">
		            <label for="edit-menu-item-title-<?php echo $item_id; ?>">
		                <?php _e( 'Navigation Label', 'goodlife' ); ?><br />
		                <input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
		            </label>
		        </p>
		        <p class="description description-thin">
		            <label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
		                <?php _e( 'Title Attribute', 'goodlife' ); ?><br />
		        ...
		        <p class="field-link-target description">
		            <label for="edit-menu-item-target-<?php echo $item_id; ?>">
		                ...
		                <?php _e( 'Open link in a new window/tab', 'goodlife' ); ?>
		            </label>
		        </p>
		        <p class="field-css-classes description description-thin">
		            <label for="edit-menu-item-classes-<?php echo $item_id; ?>">
		                <?php _e( 'CSS Classes (optional)', 'goodlife' ); ?><br />
		                <input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
		            </label>
		        </p>
		        <p class="field-xfn description description-thin">
		            <label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
		                <?php _e( 'Link Relationship (XFN)', 'goodlife'  ); ?><br />
		                <input type="text" id="edit-menu-item-xfn-<?php echo $item_id; ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
		            </label>
		        </p>
		        <p class="field-description description description-wide">
		            <label for="edit-menu-item-description-<?php echo $item_id; ?>">
		                <?php _e( 'Description', 'goodlife' ); ?><br />
		                <textarea id="edit-menu-item-description-<?php echo $item_id; ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo $item_id; ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
		                <span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.', 'goodlife'); ?></span>
		            </label>
		        </p>
		        ...
		        <div class="thb_menu_options">
		        	<h2><?php _e('Fuel Themes Menu Options', 'goodlife'); ?></h2>   
		          <div class="thb-field-link description description-thin">
		            <h3><?php _e( 'Menu Item Icon', 'goodlife' ); ?></h3>
		            ...
		            <select id="edit-menu-item-menuicon-<?php echo $item_id; ?>" name="edit-menu-item-menuicon[<?php echo $item_id; ?>]">
		          	...
		          	<h3><?php _e( 'Mega Menu', 'goodlife'  ); ?></h3>
		              <?php 
		      ...
		      <div class="menu-item-actions description-wide submitbox">
		          <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
		              <p class="link-to-original">
		                  <?php printf( esc_html__('Original: %s', 'goodlife'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
		              </p>
		          <?php endif; ?>
		          ...
		          ); ?>"><?php _e('Remove', 'goodlife' ); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
		              ?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('Cancel', 'goodlife'); ?></a>
		      </div>
