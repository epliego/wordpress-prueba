...
add_action('wp_ajax_nopriv_thb_import_ajax', 'thb_import_data');

function thb_import_data() {
	
	$fetch_images = $_POST['fetch_images'];
	$demo = $_POST['demo'] ? $_POST['demo'] : 'goodlife';
	// Load Importer API
	require_once ABSPATH . 'wp-admin/includes/import.php';
	$importerError = false;
  
  $file = get_template_directory() ."/inc/democontent/".$demo."/democontent.xml";
	...
	if($importerError !== false) {
		...
		if(class_exists('WP_Importer')){
			try{
				...
				thb_import_theme_options($demo);
				$importer->import($file);
				
			  thb_update_options();
			  thb_update_menus('navigation', 'nav-menu');
				thb_update_menus('navigation', 'mobile-menu');
			  thb_update_menus('top-menu', 'subheader-menu');
			  
			  thb_update_widgets($demo);
...
function thb_import_theme_options($demo) {
	$file = get_template_directory_uri() ."/inc/democontent/".$demo."/theme-options.txt";

	$theme_options_txt = wp_remote_get( $file );
	if ( is_wp_error( $theme_options_txt ) ) {
		echo $error_string = $theme_options_txt->get_error_message();
		return;
	}
...
function thb_update_widgets($demo) {
	$widgets_json = get_template_directory_uri() . "/inc/democontent/".$demo."/widget_data.json"; // widgets data file
	$widgets_json = wp_remote_get( $widgets_json );
	
	if ( is_wp_error( $widgets_json ) ) {
		echo $error_string = $widgets_json->get_error_message();
		return;
	}
